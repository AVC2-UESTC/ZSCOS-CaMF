...
from ..backbones.mit_IMGNetPretrined import MixVisionTransformer as mit_IMG
...
class Segformer_Config():
    ...
    @property
    def model(self):
        return SegFormer(**self.__dict__)
    
# b0==============================================

def segformer_b0_cfg():
        
        segformer_b0_args = Segformer_Config(
            backbone_cfg = dict(
                in_channels=3,
                embed_dims=32,
                num_stages=4,
...
def segformer_b2_cfg():
    segformer_b2_args = Segformer_Config(
        backbone_cfg = dict(
                in_channels=3,
                embed_dims=64,#
                num_layers=[3, 4, 6, 3],#
...
def segformer_b4_cfg():
    segformer_b4_args = Segformer_Config(
            backbone_cfg = dict(
                in_channels=3,
                embed_dims=64,#
                num_layers=[3, 8, 27, 3],#
...
class SegFormer_IMG(SegFormer, nn.Module):
    def __init__(self, 
                 backbone_cfg: dict, 
                 decode_head_cfg: dict, 
                 threshold=None, 
                 loss_decode=dict(loss_type='CrossEntropyLoss', reduction='mean'), ignore_index=255) -> None:
        # does not run SegFormer.__init__()
        super(SegFormer, self).__init__()
                
        self.backbone = mit_IMG(**backbone_cfg)
...
class Segformer_IMG_Config():
    ...
    @property
    def model(self):
        return SegFormer_IMG(**self.__dict__)       


# ======= b4 ===================

def segformerIMG_b4_cfg():
    segformer_b4_args = Segformer_IMG_Config(
            backbone_cfg = dict(
                in_chans=3, 
                 embed_dims=64,
                 num_heads=[1, 2, 5, 8], 
