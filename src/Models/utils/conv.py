...
def build_conv_layer(conv_type:str, 
                     in_channels,
                    out_channels,
                    kernel_size=3,
                    stride=1,
    ...
    '''
    """Build one convolution layer.

    Args:
        conv_type: support 'Conv1d', 'Conv2d', 'Conv3d'
    ...
    layer = supported_conv[conv_type](in_channels, out_channels, kernel_size, stride, padding, dilation, groups, bias= bias)
...
class ConvModule(nn.Module):
    ...
    def __init__(self,
                 ...
                 conv_cfg: Optional[Dict] = None, # just conv_type
        ...
        official_padding_mode = ['zeros', 'circular', 'reflect', 'replicate']
        ...
        if self.with_explicit_padding:
            pad_cfg = dict(padding_type=padding_mode, padding = padding)
        ...
        if self.with_norm:
            ...
            self.norm_name, norm = build_norm_layer(
                norm_channels, **norm_cfg)  # type: ignore
    ...
    def init_weights(self):
        ...
        if not hasattr(self.conv, 'init_weights'):
            if self.with_activation and self.act_cfg['act_type'] == 'LeakyReLU':
                nonlinearity = 'leaky_relu'
                a = self.act_cfg.get('layer_args', 0.01)
            else:
                nonlinearity = 'relu'
                a = 0
